...
- **区域管理**：支持自定义深渊区域和豁免区
- **数据持久化**：玩家数据自动保存和加载
- **成就系统**：完整的深渊成就树，从赤笛到奈落之底
- **视觉效果系统**：层级滤镜、视觉特效等沉浸式体验
- **音效系统**：第三层随机音效，增强氛围

### 诅咒层级

1. **第一层 - 阿比斯之渊** (96 > y >= 85)：反胃 + 视觉滤镜
...
- **白笛**：成为传说（挑战类）
- **绝界行**：有去无回（挑战类）
- **来自深渊**：回归深渊（挑战类，生骸转换时获得）
- **奈落之底**：故事不会从这里结束（挑战类）

成就通过游戏内进度系统自动触发，无需手动授予。
...
- 插件每 1 秒（20 tick）检查一次玩家 Y 坐标变化
- 上升时累计上升高度增加
- 下降时累计上升高度减少
- 累计上升高度达到配置的阈值（默认 2 格）时触发诅咒
- 累计上升记录在 20 分钟后自动过期

### 诅咒触发

- 触发条件：累计上升高度 >= 配置的阈值（默认 2 格）
- 诅咒层级：根据触发时的安全高度确定（基于配置的层级范围）
- 持续时间：根据诅咒臂动态计算（10分钟 * (半径 - 诅咒臂) / 半径）
- 诅咒效果：根据层级施加不同的负面效果
- 视觉效果：第一层触发视觉滤镜，第三层播放随机音效
