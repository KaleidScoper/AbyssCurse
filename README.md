...
诅咒只以 abyss 区域内的玩家为"目标"。

允许管理员玩家用两个角点（必须给出完整 xyz 坐标）来定义豁免区，豁免区内的目标不受诅咒影响，即使豁免区位于 abyss 内部。

允许管理员玩家将某个玩家定义为豁免者（称为生骸，narehate）。豁免者在任何情况下都不受诅咒影响。
...
- **触发条件**：累计上升高度 >= 2 格（可配置）
- **诅咒层级**：根据触发时的安全高度确定（基于配置的层级范围）
- **持续时间**：根据诅咒臂动态计算，公式为：**10分钟 × (abyss半径 - 诅咒臂) / abyss半径**
- **诅咒效果**：根据层级施加不同的负面效果（详见上方"诅咒层级"部分）
- **触发音效**：任何层级的诅咒触发时，都向目标玩家播放"远古守卫者施加挖掘疲劳时"的音效

### 层级效果

除了诅咒之外，每一层还带有"效果"，施加给高度位于其范围的玩家，某些效果只要没脱离范围就永久生效，具体如下：
