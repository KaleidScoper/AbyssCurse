...
1. **模式配置**
   ...
   - abyss 模式下的中心坐标和 abyss 半径（切比雪夫距离）
...
1. **AbyssRegion (深渊区域)**
   - 中心区块坐标（从方块坐标计算）
   - **abyss 半径**（切比雪夫距离，区块单位）
   - 判断玩家是否在 abyss 内
   - **计算诅咒臂（Arm of Curse）**
     - 计算玩家所在区块与中心区块的切比雪夫距离
     - 返回距离值，用于计算诅咒持续时间
   - 支持 world 模式（整个主世界，诅咒臂始终为 0）
...
1. **PlayerCurseData (玩家诅咒数据)**
   - 安全高度（double）
   - 累计上升高度（int，0-2）
   - 累计上升记录时间（Queue<Long>，时间戳队列）
   - 当前诅咒层级（int，1-7，0表示无诅咒）
   - **诅咒生效时间（long，时间戳，触发诅咒时记录）**
   - **诅咒持续时间（long，tick数，根据诅咒臂动态计算）**
   - **触发诅咒时的诅咒臂（int，用于计算持续时间）**
   - 是否为生骸（boolean）
   - 生骸类型（LUCKY/SAD，枚举）
   - 调试模式开关（boolean）
...
1. **AbyssCurseHandler (深渊诅咒处理器)**
   - 累计上升高度计算
     ...
     - 达到阈值（2m）时触发诅咒
   - **诅咒层级判断（根据安全高度）**
     - 当玩家累计上升高度达到2m触发诅咒时，根据触发时的安全高度（safeHeight）判断诅咒层级
     - 安全高度是玩家下降或累计上升高度清零时记录的高度
     - 各层的高度范围用于判断安全高度，而不是玩家的当前高度
   - **诅咒臂（Arm of Curse）计算**
     - 计算玩家所在区块与 abyss 中心区块的切比雪夫距离
     - 诅咒臂 = max(|chunkX - centerChunkX|, |chunkZ - centerChunkZ|)
   - **动态诅咒持续时间管理**
     - 基础持续时间：10分钟（600秒）
     - 实际持续时间 = 10分钟 * (abyss半径 - 诅咒臂) / abyss半径
     - 当诅咒臂 = abyss半径时，持续时间为 0（边缘区域）
     - 当诅咒臂 = 0 时，持续时间为 10分钟（中心区域）
     - 注意：world 模式下，诅咒臂始终为 0（整个主世界视为中心）
   
2. **DecompressionSicknessHandler (减压病处理器)**
   - **重要**：该模式下的诅咒只对 abyss 区域内的玩家起作用
   - 根据玩家当前高度判断层级（与 abyss-curse 模式不同，这里使用当前高度而非安全高度）
   - 永久持续诅咒（只要玩家在对应高度范围内就持续生效）
...
1. **CurseEffectApplier (诅咒效果施加器)**
   ...
   - **动态管理诅咒持续时间**
     - 接收诅咒臂和 abyss 半径参数
     - 计算实际持续时间：10分钟 * (abyss半径 - 诅咒臂) / abyss半径
     - 存储诅咒开始时间和持续时间，定期检查是否过期
   - 效果叠加逻辑（层级越高效果越多）
   - **第一层特殊处理**：触发第一层诅咒时，显示累计上升高度2的滤镜（固定重度滤镜）
...
**重要说明**：以下各层的高度范围是用于衡量**触发诅咒时记录的安全高度**，而不是玩家的当前高度。当玩家累计上升高度达到2m触发诅咒时，系统会根据触发时的安全高度（safeHeight，可以认为是本次上升的起始高度）来判断应该施加哪一层的诅咒效果。

1. **第一层：阿比斯之渊（Edge of the Abyss）**
   - 安全高度范围：96 > y >= 85（当触发诅咒时的安全高度在此范围内时，施加第一层诅咒）
   - 诅咒内容：
     - 反胃（NAUSEA）
     - 视野笼罩累计上升高度2的滤镜（固定重度红色滤镜）
   - 持续时间：动态计算（10分钟 * (abyss半径 - 诅咒臂) / abyss半径）

2. **第二层：诱惑之森（Forest of Temptation）**
   - 安全高度范围：85 > y >= 75（当触发诅咒时的安全高度在此范围内时，施加第二层诅咒）
   - 诅咒内容：
     - 第一层所有诅咒（反胃 + 重度滤镜）
     - 饥饿3（HUNGER, amplifier=3）
     - 缓慢6（SLOWNESS, amplifier=6）
   - 持续时间：动态计算

3. **第三层：大断层（Great Fault）**
   - 安全高度范围：75 > y >= 40（当触发诅咒时的安全高度在此范围内时，施加第三层诅咒）
   - 诅咒内容：
     - 以上所有诅咒（反胃 + 重度滤镜 + 饥饿3 + 缓慢6）
     - 黑暗1（BLINDNESS, amplifier=1）
     - 随机播放音效（音效库包括：所有怪物音效、所有洞穴氛围音效、TNT点燃音效）
   - 持续时间：动态计算

4. **第四层：巨人之杯（Goblets of Giants）**
   - 安全高度范围：40 > y >= 0（当触发诅咒时的安全高度在此范围内时，施加第四层诅咒）
   - 诅咒内容：
     - 以上所有诅咒（反胃 + 重度滤镜 + 饥饿3 + 缓慢6 + 黑暗1 + 随机音效）
     - 霉运（BAD_LUCK）
     - 挖掘疲劳3（MINING_FATIGUE, amplifier=3）
     - 虚弱（WEAKNESS）
   - 持续时间：动态计算

5. **第五层：亡骸之海（Sea of Corpses）**
   - 安全高度范围：0 > y >= -8（当触发诅咒时的安全高度在此范围内时，施加第五层诅咒）
   - 诅咒内容：
     - 以上所有诅咒（反胃 + 重度滤镜 + 饥饿3 + 缓慢6 + 黑暗1 + 随机音效 + 霉运 + 挖掘疲劳3 + 虚弱）
     - 失明（BLINDNESS，注意：与第三层的黑暗1叠加）
     - 禁止使用右键（监听 PlayerInteractEvent，取消右键交互）
     - 游戏静音（客户端设置，需要特殊处理）
     - 中毒2（POISON, amplifier=2）
     - 聊天栏不可见（监听聊天事件，隐藏聊天消息）
   - 持续时间：动态计算

6. **第六层：来无还之都（Capital of the Unreturned）**
   - 安全高度范围：-8 > y >= -28（当触发诅咒时的安全高度在此范围内时，施加第六层诅咒）
   - 诅咒内容：
     - 以上全部诅咒（包括第五层的所有内容）
     - 寄生（虫蚀，具体用法有待参考官方文档）
     - 凋零2（WITHER, amplifier=2）
   - 持续时间：动态计算
   - **特殊机制**：诅咒结束后未死亡则自动转换为生骸（豁免者）

7. **第七层：最终极之涡（Final Maelstrom）**
   - 安全高度范围：-28 > y >= -64（当触发诅咒时的安全高度在此范围内时，施加第七层诅咒）
   - 诅咒内容：
     - 以上全部诅咒（包括第六层的所有内容）
     - 瞬间伤害2（INSTANT_DAMAGE, amplifier=2）
     - 诅咒结束后未死亡则强制击杀
   - 持续时间：动态计算

**各层层级效果详细列表**（永久生效，只要玩家在对应高度范围内）：

1. **第一层：阿比斯之渊**
   - 高度范围：96 > y >= 85
   - Title：红色 "阿比斯之渊"
   - Subtitle："Edge of the Abyss"
   - 效果：生命恢复（REGENERATION）
   - 成就：赤笛（进入时触发）
...
2. **第二层：诱惑之森**
   ...
   - 效果：伤害吸收1（ABSORPTION, amplifier=1）
   - 成就：苍笛（进入时触发）

3. **第三层：大断层**
   - 高度范围：75 > y >= 40
   - Title：灰色 "大断层"
   - Subtitle："Great Fault"
   - 效果：抗性提升（RESISTANCE）
   - 成就：月笛（进入时触发）
...
4. **第四层：巨人之杯**
   ...
   - 效果：夜视（NIGHT_VISION）+ 急迫（HASTE）+ 迅捷（SPEED）+ 力量（STRENGTH）
   - 成就：黑笛（进入时触发）
...
5. **第五层：亡骸之海**
   ...
   - 效果：无
   - 成就：白笛（进入时触发，挑战类进度，音效：ui.toast.challenge_complete）

6. **第六层：来无还之都**
   - 高度范围：-8 > y >= -28
   - Title：金色 "来无还之都"
   - Subtitle："Capital of the Unreturned"
   - 效果：幸运（LUCK）
   - 限制：不再允许打开聊天栏（但可以看见内容）
   - 成就：绝界行（进入时触发，挑战类进度，音效：ui.toast.challenge_complete）

7. **第七层：最终极之涡**
   - 高度范围：-28 > y >= -64
   - Title：白色 "最终极之涡"
   - Subtitle："Final Maelstrom"
   - 效果：缓降（SLOW_FALLING）
   - 限制：不再允许打开聊天栏（但可以看见内容）
   - 成就：奈落之底（进入时触发，挑战类进度，音效：ui.toast.challenge_complete）

**注意事项**：
- 诅咒触发时播放音效："远古守卫者施加挖掘疲劳时"的音效（Sound.ENTITY_ELDER_GUARDIAN_CURSE）
- 诅咒持续时间根据诅咒臂动态计算，边缘区域持续时间更短
- 层级效果是永久性的，只要玩家在对应高度范围内就持续生效
- 第五层和第六、七层有特殊限制（禁止右键、游戏静音、聊天栏限制）
